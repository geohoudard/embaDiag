function saveResults() {
    ...
    const rows = cuveeData.map(cuvee => [
        ...
        cuvee.diagnostic.etiquetteEcoInk ? 'Oui' : 'Non',
        cuvee.diagnostic.etiquetteMat || "",
        cuvee.diagnostic.etiquettecontreMat || "",
        cuvee.diagnostic.papierreshum ? 'Oui' : 'Non',
        ...
        cuvee.diagnostic.etuissilkpaper ? 'Oui' : 'Non',
    ...
    cuveeData = values.map((value, index) => {
        return {
            ...
            diagnostic: {
                ...
                etiquetteEcoInk: value[15].replace(/"/g, '')=== 'Oui',
                etiquetteMat: value[16].replace(/"/g, ''),
                etiquettecontreMat: value[17].replace(/"/g, ''),
                papierreshum: value[18].replace(/"/g, '')=== 'Oui',
                etiquetteDor: value[19].replace(/"/g, ''),
                etiquetteColle: value[20].replace(/"/g, ''),
                
                etuisType: value[21].replace(/"/g, ''),
                etuiWeight: value[22].replace(/"/g, ''),
                elementsassos: value[23].replace(/"/g, '') === 'Oui',
                etuisEcoink: value[24].replace(/"/g, '') === 'Oui',
                etuissilkpaper: value[25].replace(/"/g, '') === 'Oui',
                etuisPapier: value[26].replace(/"/g, '') === 'Oui',
                etuisCarton: value[27].replace(/"/g, '') === 'Oui',
                etuisBois: value[28].replace(/"/g, '') === 'Oui',
                etuisPlastique: value[29].replace(/"/g, '') === 'Oui',
                etuisAimant: value[30].replace(/"/g, '') === 'Oui',

                suremballage: value[31].replace(/"/g, ''),
                suremballageEcoink: value[32].replace(/"/g, '') === 'Oui',
                sacPapier: value[33].replace(/"/g, '') === 'Oui',
                sacCarton: value[34].replace(/"/g, '') === 'Oui',
                sacPlastique: value[35].replace(/"/g, '') === 'Oui',
                sacAimant: value[36].replace(/"/g, '') === 'Oui',

                cartonRecycled: value[37].replace(/"/g, ''),
                cartonCannelure: value[38].replace(/"/g, ''),
                cartonInter: value[39].replace(/"/g, ''),
                cartonScotch: value[40].replace(/"/g, ''),
                cartonDor: value[41].replace(/"/g, ''),
                cartonInk: value[42].replace(/"/g, ''),

                objet: value[43].replace(/"/g, '')
            }
        };
    });
